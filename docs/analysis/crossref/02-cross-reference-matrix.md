...
| Workspace lifecycle + path safety | Local/remote path validation + hook semantics + cleanup resilience (`workspace.ex`, `path_safety.ex`) | Strict local containment/cwd invariants + hook timeout/failure model, plus first-class provisioner strategies (`worktree`/`clone`/`none`) with deterministic branch templating, dirty-repo policy, teardown controls, and additive provision diagnostics (`src/workspace/manager.ts`, `src/workspace/provisioner.ts`) | `functionally-equivalent` local semantics with additive provisioner parity | `spec-required` + `extension` | Low | Keep; remote-aware parity requirements are closed in P11 and workspace provisioner uplift is closed in P19 |
| HTTP API contract | Phoenix router/controller/presenter for `/`, `/api/v1/state`, `/api/v1/refresh`, `/api/v1/:issue_identifier` and LiveView-driven push updates | Local HTTP server with matching core endpoints plus `GET /api/v1/events` SSE, typed snapshot error payloads, diagnostics/history/ui-state endpoints, runtime workflow controls, full workspace/worker projection parity, additive retry-causality/runtime-resolution diagnostics surfaces, and blocked-input projections with manual resume endpoint (`POST /api/v1/issues/:issue_identifier/resume`) (`src/api/server.ts`, `src/api/types.ts`, `src/api/snapshot-service.ts`) | `different-safe`; ours superset with explicit stream + operator diagnostics contract | `spec-required` + `extension` | Low | Preserve superset and maintain envelope compatibility (`XR-03`) |
...
2. Previously high-priority parity deltas XR-01, XR-02, XR-04, XR-05, and XR-08 are closed in P11; post-P15 protocol/API/runtime parity deltas are closed in P16, test-behavior parity closure is delivered in P17, workspace-resolution/operator-clarity deltas are closed in P18, and first-class workspace provisioner parity is closed in P19.
