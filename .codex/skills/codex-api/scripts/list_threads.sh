...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --sort) SORT="${2:-updated}"; shift 2 ;;
    --direction) DIRECTION="${2:-desc}"; shift 2 ;;
...
QUERY_PARTS=("limit=${LIMIT}" "offset=${OFFSET}" "sort=${SORT}" "direction=${DIRECTION}")
...
"${DIR}/api_client.sh" GET /threads --query "$QUERY"
