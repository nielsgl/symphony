...
import { IdentityProjectionStore, parseDurableIdentity, serializeDurableIdentity } from './identity-projection-store';
...
export class SqlitePersistenceStore {
  ...
  listRunHistory(limit = 50): DurableRunHistoryRecord[] {
    ...
    return rows.map((row) => {
      ...
      const identityProjection = this.identityProjectionStore.readHistoryIdentityProjection(identityProjectionStmt, row.run_id);
