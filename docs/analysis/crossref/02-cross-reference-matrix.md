...
|---|---|---|---|---|---|---|
| Config/workflow schema | Ecto embedded schema with runtime policy resolution and String-or-map approval policy (`config/schema.ex`, `config.ex`) | Typed resolver/validator with strict error codes, including GitHub state validation, worker host cap validation, strict fail-fast numeric validation for provided invalid core fields, string-or-object approval policy normalization, parity options (`tracker.assignee`, `server.host`, `observability.*`), and workflow-relative workspace-root resolution metadata (`workspace.root_source`) (`src/workflow/resolver.ts`, `src/workflow/validator.ts`) | `equivalent` for policy-shape compatibility and runtime path resolution semantics | `spec-required` | Low | Keep; XR-02 closed in P11, parity knobs delivered in P15b, workspace resolution clarity closed in P18, and strict numeric validation parity refreshed in P21 |
